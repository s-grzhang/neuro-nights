...
const GoalsPage = ({ userId, userData, updateUserData, onEarnPoints, goals: initialGoals, setGoals: updateAppGoals }) => {
  ...
  const handleSetGoal = async (e) => {
    ...
    try {
      ...
      if (editingGoal) {
        ...
        if (typeof updateAppGoals === 'function') {
          // Create obj in the format App component uses (text by type)
          const appGoalFormat = {
            [goalType]: goalText
          };
          updateAppGoals(prevGoals => ({
            ...prevGoals,
            ...appGoalFormat
          }));
        }
      ...
      setSuccess(`Goal successfully ${editingGoal ? 'updated' : 'added'}.`);
    ...
    try {
      ...
      switch (type) {
        ...
        case 'consistency':
          ...
          break;
        case 'bedtime':
          // Handle both formats of the recommendations (for backward compatibility)
          const start = recommendations.bedtime.start || recommendations.bedtime.bedtime;
          const end = recommendations.bedtime.end || recommendations.bedtime.wakeTime;
          goalText = `I will sleep from ${start} to ${end} each night.`;
          setBedtime(start);
          setWakeTime(end);
          break;
        default:
  ...
  const handleDeleteGoal = async (goalId) => {
    try {
      // Get the goal to be deleted so we know its type
      ...
      // Update App component state if the goal type is known
      if (goalToDelete && goalToDelete.type && typeof updateAppGoals === 'function') {
        updateAppGoals(prevGoals => {
          const newGoals = { ...prevGoals };
          // Set this goal type to "No goal set" message
          newGoals[goalToDelete.type] = `No ${goalToDelete.type} goal set.`;
          return newGoals;
        });
      }
  ...
  const getRecommendedGoalText = (type) => {
    ...
    switch (type) {
      ...
      case 'bedtime':
        ...
        const start = recommendations.bedtime.start || recommendations.bedtime.bedtime;
        const end = recommendations.bedtime.end || recommendations.bedtime.wakeTime;
        return `Sleep from ${start} to ${end}`;
      default:
        return '';
    }
  };
  ...
  useEffect(() => {
    console.log("GoalsPage - Auth Status:", {
  ...
  // Initialize with initialGoals if provided
  useEffect(() => {
    if (initialGoals && Object.keys(initialGoals).length > 0) {
      // Convert initialGoals object to array format that component uses
      const goalsArray = Object.entries(initialGoals).map(([key, value], index) => ({
        id: `initial-${index}`,
        text: value,
        type: key,
        progress: 0,
